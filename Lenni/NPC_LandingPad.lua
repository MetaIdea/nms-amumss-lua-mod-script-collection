...
{
	{ ["ID_NAME"]="_U_NPCLANDING", ["P_NAME"]="NPCLANDINGPAD", ["PATH"]="MODELS/PLANETS/BIOMES/COMMON/BUILDINGS/PARTS/BUILDABLEPARTS/TECH/", ["ICON"]=BLDGICON, ["NAME"]= "NPC_LANDING_NAME", ["NAMELOWER"]="NPC_LANDING_NAME_L", ["DESCRIPTION"]="NPC_LANDING_DESC" },
	{ ["ID_NAME"]="ALT_NPCLANDING", ["P_NAME"]="NPCLANDINGZONE", ["PATH"]="MODELS/PLANETS/BIOMES/COMMON/BUILDINGS/PARTS/BUILDABLEPARTS/TECH/", ["ICON"]=ALTBLDGICON, ["NAME"]="NPC_LANDING_ALT_NAME", ["NAMELOWER"]="NPC_LANDING_ALT_NAME_L", ["DESCRIPTION"]="NPC_LANDING_DESC" },
}

function GetBaseBuildingObject(ID_NAME, PATH, P_NAME)
...
for i=1,#BUILDABLE_OBJECT_TABLE,1 do
	local baseBuildingParts = GetBaseBuildingPart(BUILDABLE_OBJECT_TABLE[i]["ID_NAME"], BUILDABLE_OBJECT_TABLE[i]["P_NAME"], BUILDABLE_OBJECT_TABLE[i]["PATH"])
	for j=1, #baseBuildingParts, 1 do
		table.insert(BASEBUILDINGPARTSTABLE_ADDING_ALL, baseBuildingParts[j])
	end

	local baseBuildingObjects = GetBaseBuildingObject(BUILDABLE_OBJECT_TABLE[i]["ID_NAME"], BUILDABLE_OBJECT_TABLE[i]["P_NAME"], BUILDABLE_OBJECT_TABLE[i]["PATH"])
