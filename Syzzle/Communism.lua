Author = "Syzzle"
ModName = "Communism"
GameVersion = "6.10"
Description = "Makes every Expedition and Twitch item added by Consumerism FREE OF CHARGE."

ProductIDList =
{
  {"ID","BASE_HOTPLANT04"},
  {"ID","BASE_HOTPLANT05"},
  {"ID","BASE_HOTPLANT06"},
  {"ID","BASE_JUNGLE04"},
  {"ID","BASE_RADPLANT04"},
  {"ID","BASE_SUBZPLANT9"},
  {"ID","BASE_TOXPLANT04"},
  {"ID","BASE_WPLANT7"},
  {"ID","BASE_WPLANT8"},
  {"ID","EXPD_BANNER20"},
  {"ID","EXPD_DECAL20"},
  {"ID","EXPD_POSTER20A"},
  {"ID","EXPD_POSTER20B"},
  {"ID","EXPD_POSTER20C"},
  {"ID","EXPD_TITLE20"},
  {"ID","SHIP_ATLAS"},
  {"ID","EXPD_BANNER19"},
  {"ID","EXPD_DECAL19"},
  {"ID","EXPD_EGG_19"},
  {"ID","EXPD_POSTER19A"},
  {"ID","EXPD_POSTER19B"},
  {"ID","EXPD_POSTER19C"},
  {"ID","EXPD_TITLE19"},
  {"ID","JETS_RUIN"},
  {"ID","S19_TP"},
  {"ID","SHIP_PLASMA"},
  {"ID","TWITCH_FIREW11A"},
  {"ID","TWITCH_GUN25"},
  {"ID","TWITCH_GUN26"},
  {"ID","TWITCH_GUN27"},
  {"ID","TWITCH_PET23"},
  {"ID","TWITCH_PET24"},
  {"ID","TWITCH_SHIP51"},
  {"ID","TWITCH_SHIP52"},
  {"ID","TWITCH_SHIP53"},
  {"ID","TWITCH_SHIP54"},
  {"ID","TWITCH_SHIP55"},
  {"ID","DECAL_BONESOUP"},
  {"ID","DECAL_EXTINCT"},
  {"ID","DECAL_SCAN"},
  {"ID","DECAL_BONEJUDGE"},
  {"ID","DECAL_RUINMECH"},
  {"ID","BASE_JUNGLE03"},
  {"ID","BASE_JUNGLE02"},
  {"ID","BASE_JUNGLE01"},
  {"ID","EXPD_POSTER18A"},
  {"ID","EXPD_POSTER18B"},
  {"ID","EXPD_POSTER18C"},
  {"ID","EXPD_TITLE18"},
  {"ID","EXPD_DECAL18"},
  {"ID","EXPD_BANNER18"},
  {"ID","EXPD_EGG_18"},
  {"ID","EXPD_STAFF_18"},
  {"ID","EXPD_STONE_PACK"},
  {"ID","STONE_HAT"},
  {"ID","STONE_ARMOUR"},
  {"ID","STONE_TORSO"},
  {"ID","STONE_GLOVES"},
  {"ID","STONE_LEGS"},
  {"ID","STONE_BOOTS"},
  {"ID","EXPD_BANNER17"},
  {"ID","EXPD_CAPE17"},
  {"ID","EXPD_DECAL17"},
  {"ID","EXPD_POSTER17A"},
  {"ID","EXPD_POSTER17B"},
  {"ID","EXPD_POSTER17C"},
  {"ID","EXPD_SHIP17"},
  {"ID","EXPD_SHIPMAT17"},
  {"ID","EXPD_STAFF_17"},
  {"ID","EXPD_TITLE17"},
  {"ID","SPEC_FISH_BOB1"},
  {"ID","SPEC_FISH_BOB2"},
  {"ID","SPEC_FISH_BOB3"},
  {"ID","SPEC_FISH_BOB4"},
  {"ID","SPEC_FISH_BOB5"},
  {"ID","SPEC_FISH_BOB6"},
  {"ID","SPEC_FISH_HAT1"},
  {"ID","SPEC_FISH_HAT2"},
  {"ID","SPEC_FISH_HAT3"},
  {"ID","SPEC_FISH_HAT4"},
  {"ID","SPEC_FISH_HAT5"},
  {"ID","SPEC_FISH_HAT6"},
  {"ID","TWITCH_FIREW10A"},
  {"ID","TWITCH_FIREW10B"},
  {"ID","TWITCH_FIREW10C"},
  {"ID","TWITCH_FIREW10D"},
  {"ID","TWITCH_FIREW10E"},
  {"ID","TWITCH_GUN22"},
  {"ID","TWITCH_GUN23"},
  {"ID","TWITCH_GUN24"},
  {"ID","TWITCH_PET21"},
  {"ID","TWITCH_PET22"},
  {"ID","TWITCH_SHIP46"},
  {"ID","TWITCH_SHIP47"},
  {"ID","TWITCH_SHIP48"},
  {"ID","TWITCH_SHIP49"},
  {"ID","TWITCH_SHIP50"},
  {"ID","TGA_SHIP01"},
  {"ID","DECAL_PEEP"},
  {"ID","BANNER_PEEP"},
  {"ID","BASE_SUBZPLANT5"},
  {"ID","BASE_SUBZPLANT6"},
  {"ID","BASE_SUBZPLANT7"},
  {"ID","BASE_SUBZPLANT8"},
  {"ID","PORTAL_JUICE"},
  {"ID","BOSS_JUICE"},
  {"ID","EXPD_SHIP16"},
  {"ID","JETS_SQUID"},
  {"ID","EXPD_S16_HAT"},
  {"ID","EXPD_EGG_16"},
  {"ID","EXPD_POSTER16A"},
  {"ID","EXPD_POSTER16B"},
  {"ID","EXPD_POSTER16C"},
  {"ID","EXPD_TITLE16"},
  {"ID","EXPD_DECAL16"},
  {"ID","EXPD_BANNER16"},
  {"ID","S16_HEATER"},
  {"ID","BANNER_TRA"},
  {"ID","BANNER_WAR"},
  {"ID","BANNER_EXP"},
  {"ID","BASE_PEACOCK01"},
  {"ID","BASE_PEACOCK02"},
  {"ID","BASE_PEACOCK03"},
  {"ID","BASE_FOXGLOVE01"},
  {"ID","BASE_FLORAL01"},
  {"ID","BASE_FLORAL02"},
  {"ID","BASE_FLORAL03"},
  {"ID","BASE_FLORAL04"},
  {"ID","DECAL_NMS_FR"},
  {"ID","BANNER_NMS_FR"},
  {"ID","DECAL_NMSCORD"},
  {"ID","BANNER_NMSCORD"},
  {"ID","DECAL_UN42"},
  {"ID","BANNER_UN42"},
  {"ID","DECAL_AF"},
  {"ID","BANNER_AF"},
  {"ID","BASE_SUBZPLANT1"},
  {"ID","BASE_SUBZPLANT2"},
  {"ID","BASE_SUBZPLANT3"},
  {"ID","BASE_SUBZPLANT4"},
  {"ID","BASE_SPRIGBUSH"},
  {"ID","BASE_FLYINGFERN"},
  {"ID","EXPD_POSTER15A"},
  {"ID","EXPD_POSTER15B"},
  {"ID","EXPD_POSTER15C"},
  {"ID","EXPD_FISH_PACK"},
  {"ID","EXPD_S15_ARMOUR"},
  {"ID","EXPD_S15_HAT"},
  {"ID","EXPD_S15_LEGS"},
  {"ID","EXPD_S15_BOOTS"},
  {"ID","EXPD_TITLE15"},
  {"ID","EXPD_DECAL15"},
  {"ID","EXPD_BANNER15"},
  {"ID","EXPD_S15_LASER"},
  {"ID","BOBBLE_OCTO"},
  {"ID","EXPED_BUG_ARMOR"},
  {"ID","EXPED_BUG_GLOVE"},
  {"ID","EXPED_BUG_BOOTS"},
  {"ID","EXPED_BUG_HAT"},
  {"ID","EXPED_BUG_PACK"},
  {"ID","EXPD_EGG_14"},
  {"ID","BLD_BUGHEAD1"},
  {"ID","BLD_BUGHEAD2"},
  {"ID","BLD_BUGHEAD3"},
  {"ID","BLD_BUGHEAD4"},
  {"ID","BLD_BUGHEAD5"},
  {"ID","EXPD_POSTER14A"},
  {"ID","EXPD_POSTER14B"},
  {"ID","EXPD_POSTER14C"},
  {"ID","EXPD_TITLE14"},
  {"ID","EXPD_DECAL14"},
  {"ID","EXPD_BANNER14"},
  {"ID","TWITCH_SHIP41"},
  {"ID","TWITCH_SHIP42"},
  {"ID","TWITCH_SHIP43"},
  {"ID","TWITCH_SHIP44"},
  {"ID","TWITCH_SHIP45"},
  {"ID","TWITCH_PET20"},
  {"ID","TWITCH_GUN19"},
  {"ID","TWITCH_GUN20"},
  {"ID","TWITCH_GUN21"},
  {"ID","SPEC_FIREWORK10"},
  {"ID","SPEC_FIREWORK11"},
  {"ID","SPEC_FIREWORK12"},
  {"ID","EXPD_POSTER13A"},
  {"ID","EXPD_POSTER13B"},
  {"ID","EXPD_POSTER13C"},
  {"ID","BLD_SHIPBREAK1"},
  {"ID","BLD_SHIPBREAK2"},
  {"ID","BLD_SHIPBREAK3"},
  {"ID","BLD_SHIPBREAK4"},
  {"ID","BLD_SHIPBREAK5"},
  {"ID","EXPD_SHIPMAT13"},
  {"ID","EXPD_SHIP_METAL"},
  {"ID","EXPD_TITLE13"},
  {"ID","EXPD_DECAL13"},
  {"ID","EXPD_BANNER13"},
  {"ID","EXPD_EGG_13"},
  {"ID","EXPD_GHOSTFRIG"},
  {"ID","EXPD_SHIP13"},
  {"ID","S_POSTER29"},
  {"ID","S_POSTER30"},
  {"ID","S_POSTER31"},
  {"ID","BUGMECH_ARML"},
  {"ID","BUGMECH_LEGS"},
  {"ID","BUGMECH_HEAD"},
  {"ID","BUGMECH_ARMR"},
  {"ID","SPEC_FIREWORK07"},
  {"ID","SPEC_FIREWORK08"},
  {"ID","SPEC_FIREWORK09"},
  {"ID","EXPD_SHIP12"},
  {"ID","EXPD_ATLASHELM"},
  {"ID","EXPD_ATLASSTAFF"},
  {"ID","EXPD_ATLASPACK"},
  {"ID","EXPD_POSTER12A"},
  {"ID","EXPD_POSTER12B"},
  {"ID","EXPD_POSTER12C"},
  {"ID","EXPD_TITLE12"},
  {"ID","EXPD_DECAL12"},
  {"ID","EXPD_BANNER12"},
  {"ID","TWITCH_SHIP36"},
  {"ID","TWITCH_SHIP37"},
  {"ID","TWITCH_SHIP38"},
  {"ID","TWITCH_SHIP39"},
  {"ID","TWITCH_SHIP40"},
  {"ID","TWITCH_PET18"},
  {"ID","TWITCH_PET19"},
  {"ID","TWITCH_GUN16"},
  {"ID","TWITCH_GUN17"},
  {"ID","TWITCH_GUN18"},
  {"ID","DECAL_TOSTARS"},
  {"ID","DECAL_FPLANET2"},
  {"ID","DECAL_CREPAWS"},
  {"ID","STATUE_ATLAS_I"},
  {"ID","STATUE_DIP_I"},
  {"ID","STATUE_SHIP_I"},
  {"ID","DECAL_CREFLY"},
  {"ID","DECAL_CREBEETLE"},
  {"ID","DECAL_BLUESYS"},
  {"ID","STATUE_ASTRO_I"},
  {"ID","STATUE_BLOB_I"},
  {"ID","STATUE_GEK_I"},
  {"ID","STATUE_WALK_I"},
  {"ID","BLD_CRANE"},
  {"ID","BLD_COVCRATE"},
  {"ID","BLD_COVSILOS"},
  {"ID","DECAL_SPOOKATL"},
  {"ID","DECAL_SHIP1"},
  {"ID","DECAL_PIRATESYS"},
  {"ID","EXPD_POSTER11A"},
  {"ID","EXPD_POSTER11B"},
  {"ID","EXPD_POSTER11C"},
  {"ID","EXPD_TITLE11"},
  {"ID","EXPD_DECAL11"},
  {"ID","EXPD_BANNER11"},
  {"ID","JETS_ELECTRIC"},
  {"ID","EXPD_EGG_11"},
  {"ID","EXPD_PETCUST11"},
  {"ID","BLD_BUI_HAND"},
  {"ID","BLD_BUI_HEAD"},
  {"ID","BLD_BUI_TENT"},
  {"ID","BUI_HEAD_1"},
  {"ID","BUI_HEAD_2"},
  {"ID","BUI_HEAD_3"},
  {"ID","BUI_HEAD_4"},
  {"ID","BUI_HEAD_5"},
  {"ID","BUI_HEAD_6"},
  {"ID","BUI_HEAD_7"},
  {"ID","BUI_HEAD_8"},
  {"ID","BUI_HEAD_9"},
  {"ID","BUI_HEAD_10"},
  {"ID","BUI_HEAD_11"},
  {"ID","BUI_HEAD_12"},
  {"ID","BUI_HEAD_13"},
  {"ID","BUI_HEAD_14"},
  {"ID","BUI_HEAD_15"},
  {"ID","BUI_ARMOUR_1"},
  {"ID","BUI_ARMOUR_2"},
  {"ID","BUI_ARMOUR_3"},
  {"ID","BUI_ARMOUR_4"},
  {"ID","BUI_CLOTH_1"},
  {"ID","BUI_CLOTH_2"},
  {"ID","BUI_CLOTH_3"},
  {"ID","TWITCH_SHIP31"},
  {"ID","TWITCH_SHIP32"},
  {"ID","TWITCH_SHIP33"},
  {"ID","TWITCH_SHIP34"},
  {"ID","TWITCH_SHIP35"},
  {"ID","TWITCH_PET17"},
  {"ID","TWITCH_GUN13"},
  {"ID","TWITCH_GUN14"},
  {"ID","TWITCH_GUN15"},
  {"ID","TWITCH_FIREW07A"},
  {"ID","TWITCH_FIREW07B"},
  {"ID","TWITCH_FIREW07C"},
  {"ID","TWITCH_FIREW07D"},
  {"ID","TWITCH_FIREW07E"},
  {"ID","EXPD_POSTER10A"},
  {"ID","EXPD_POSTER10B"},
  {"ID","EXPD_POSTER10C"},
  {"ID","BLD_CRYS_SENT0"},
  {"ID","BLD_CRYS_SENT1"},
  {"ID","BLD_CRYS_DRONE"},
  {"ID","SPEC_ROCKETHAT"},
  {"ID","EXPD_TITLE10"},
  {"ID","EXPD_DECAL10"},
  {"ID","EXPD_BANNER10"},
  {"ID","FREI_TRAIL_RED"},
  {"ID","JETS_CORRUPT"},
  {"ID","S10_PART1A"},
  {"ID","S10_PART1B"},
  {"ID","S10_PART2"},
  {"ID","S10_PART3"},
  {"ID","S10_PART4"},
  {"ID","S10_PART5"},
  {"ID","JETS_FIRE"},
  {"ID","JETS_CONFETTI"},
  {"ID","JETS_FIREWORK"},
  {"ID","JETS_PLASMA"},
  {"ID","JETS_FUSION"},
  {"ID","SPEC_EMOTE11"},
  {"ID","SPEC_EMOTE12"},
  {"ID","SPEC_EMOTE13"},
  {"ID","SPEC_EMOTE14"},
  {"ID","SPEC_EMOTE15"},
  {"ID","CAPE_SEED"},
  {"ID","EXPD_SHIP09"},
  {"ID","EXPD_BANNER09"},
  {"ID","EXPD_DECAL09"},
  {"ID","EXPD_TITLE09"},
  {"ID","EXPD_EGG_09"},
  {"ID","SPEC_BUCKETHELM"},
  {"ID","BLD_HOLOGEK"},
  {"ID","EXPD_POSTER09C"},
  {"ID","EXPD_POSTER09B"},
  {"ID","EXPD_POSTER09A"},
  {"ID","TWITCH_SHIP26"},
  {"ID","TWITCH_SHIP27"},
  {"ID","TWITCH_SHIP28"},
  {"ID","TWITCH_SHIP29"},
  {"ID","TWITCH_SHIP30"},
  {"ID","TWITCH_PET15"},
  {"ID","TWITCH_PET16"},
  {"ID","TWITCH_GUN11"},
  {"ID","TWITCH_GUN12"},
  {"ID","TWITCH_FIREW06A"},
  {"ID","TWITCH_FIREW06B"},
  {"ID","TWITCH_FIREW06C"},
  {"ID","TWITCH_FIREW06D"},
  {"ID","TWITCH_FIREW06E"},
  {"ID","HOOD_GEK"},
  {"ID","S_SIGN_BAR1"},
  {"ID","S_SIGN_BAR2"},
  {"ID","S_SIGN_BAR3"},
  {"ID","S_POSTER26"},
  {"ID","S_POSTER27"},
  {"ID","S_POSTER28"},
  {"ID","CAPE_JELLY"},
  {"ID","EXPD_EGG_08"},
  {"ID","TECHBOX"},
  {"ID","INVBOX"},
  {"ID","SWITCH_SHIP01"},
  {"ID","SWITCH_GUN01"},
  {"ID","TWITCH_SHIP21"},
  {"ID","TWITCH_SHIP22"},
  {"ID","TWITCH_SHIP23"},
  {"ID","TWITCH_SHIP24"},
  {"ID","TWITCH_SHIP25"},
  {"ID","TWITCH_PET13"},
  {"ID","TWITCH_PET14"},
  {"ID","TWITCH_GUN09"},
  {"ID","TWITCH_GUN10"},
  {"ID","TWITCH_FIREW05A"},
  {"ID","TWITCH_FIREW05B"},
  {"ID","TWITCH_FIREW05C"},
  {"ID","TWITCH_FIREW05D"},
  {"ID","TWITCH_FIREW05E"},
  {"ID","BASE_WPLANT4"},
  {"ID","BASE_WPLANT5"},
  {"ID","BASE_WPLANT6"},
  {"ID","CAPE_BIOFRIG"},
  {"ID","BASE_FLOWER1"},
  {"ID","BASE_FLOWER2"},
  {"ID","BASE_FLOWER3"},
  {"ID","CAPE_WORM"},
  {"ID","EXPD_BANNER08"},
  {"ID","EXPD_DECAL08"},
  {"ID","EXPD_TITLE08"},
  {"ID","BLD_DATASIGN"},
  {"ID","BLD_FIREPIT"},
  {"ID","BLD_MAPTUBE"},
  {"ID","EXPD_CAPE08"},
  {"ID","FREI_TRAIL_GRAV"},
  {"ID","EXPD_POSTER08C"},
  {"ID","EXPD_POSTER08B"},
  {"ID","EXPD_POSTER08A"},
  {"ID","TWITCH_PET11"},
  {"ID","TWITCH_PET12"},
  {"ID","TWITCH_SHIP16"},
  {"ID","TWITCH_SHIP17"},
  {"ID","TWITCH_SHIP18"},
  {"ID","TWITCH_SHIP19"},
  {"ID","TWITCH_SHIP20"},
  {"ID","TWITCH_GUN06"},
  {"ID","TWITCH_GUN07"},
  {"ID","TWITCH_GUN08"},
  {"ID","BASE_GRASS5"},
  {"ID","BASE_GRASS6"},
  {"ID","BASE_GRASS7"},
  {"ID","S_POSTER23"},
  {"ID","S_POSTER24"},
  {"ID","S_POSTER25"},
  {"ID","BASE_GRASS1"},
  {"ID","BASE_GRASS2"},
  {"ID","BASE_GRASS3"},
  {"ID","BASE_GRASS4"},
  {"ID","FREI_TRAIL_YEL"},
  {"ID","FREI_TRAIL_PUR"},
  {"ID","FREI_TRAIL_TUR"},
  {"ID","EXPD_POSTER07A"},
  {"ID","EXPD_POSTER07B"},
  {"ID","EXPD_POSTER07C"},
  {"ID","SHIP_ROGUE"},
  {"ID","EXPD_CAPE07"},
  {"ID","BLD_MINI_BIOFRI"},
  {"ID","EXPD_TITLE07"},
  {"ID","EXPD_DECAL07"},
  {"ID","EXPD_BANNER07"},
  {"ID","EXPD_BIOFRIG"},
  {"ID","S_POSTER20"},
  {"ID","S_POSTER21"},
  {"ID","S_POSTER22"},
  {"ID","SHIP_PIRATE"},
  {"ID","EXPD_CAPE06"},
  {"ID","SPEC_HOOD01"},
  {"ID","EXPD_CUBE_06"},
  {"ID","EXPD_TITLE06"},
  {"ID","EXPD_DECAL06"},
  {"ID","EXPD_BANNER06"},
  {"ID","EXPD_FLAG_06A"},
  {"ID","EXPD_FLAG_06B"},
  {"ID","EXPD_FLAG_06C"},
  {"ID","EXPD_FLAG_06D"},
  {"ID","EXPD_FLAG_06E"},
  {"ID","EXPD_POSTER06A"},
  {"ID","EXPD_POSTER06B"},
  {"ID","EXPD_POSTER06C"},
  {"ID","TWITCH_PET06"},
  {"ID","TWITCH_PET07"},
  {"ID","TWITCH_PET08"},
  {"ID","TWITCH_PET09"},
  {"ID","TWITCH_PET10"},
  {"ID","TWITCH_SHIP11"},
  {"ID","TWITCH_SHIP12"},
  {"ID","TWITCH_SHIP13"},
  {"ID","TWITCH_SHIP14"},
  {"ID","TWITCH_SHIP15"},
  {"ID","DECAL_WORM_P1"},
  {"ID","DECAL_WORM_P2"},
  {"ID","DECAL_WORM_P3"},
  {"ID","DECAL_WORM_P4"},
  {"ID","DECAL_WORM_P5"},
  {"ID","JETS_SENT"},
  {"ID","EXPD_POSTER05A"},
  {"ID","EXPD_POSTER05B"},
  {"ID","EXPD_POSTER05C"},
  {"ID","EXPD_PETCUST05"},
  {"ID","EXPD_BACKPACK05"},
  {"ID","EXPD_TITLE05"},
  {"ID","EXPD_DECAL05"},
  {"ID","EXPD_BANNER05"},
  {"ID","EXPD_EGG_05"},
  {"ID","PLANTPOT4"},
  {"ID","BUILDCHAIR4"},
  {"ID","S_POSTER17"},
  {"ID","S_POSTER18"},
  {"ID","S_POSTER19"},
  {"ID","DECAL_GALCORE"},
  {"ID","DECAL_UWTREAS"},
  {"ID","DECAL_NEXUSM"},
  {"ID","EXPD_FIREWORK04"},
  {"ID","EXPD_TITLE04"},
  {"ID","EXPD_DECAL04"},
  {"ID","EXPD_BANNER04"},
  {"ID","EXPD_WORMHAT"},
  {"ID","BLD_SKULL"},
  {"ID","EXPD_EGG_04"},
  {"ID","JETS_WORM"},
  {"ID","TWITCH_PET05"},
  {"ID","TWITCH_PET04"},
  {"ID","TWITCH_PET03"},
  {"ID","TWITCH_PET02"},
  {"ID","TWITCH_PET01"},
  {"ID","TWITCH_SHIP10"},
  {"ID","TWITCH_SHIP09"},
  {"ID","TWITCH_SHIP08B"},
  {"ID","TWITCH_SHIP07"},
  {"ID","TWITCH_SHIP06"},
  {"ID","BASE_BUBPIPE"},
  {"ID","EXPD_EGG_03"},
  {"ID","EXPD_TITLE03"},
  {"ID","EXPD_DECAL03"},
  {"ID","EXPD_BANNER03"},
  {"ID","BASE_CAVE1"},
  {"ID","BASE_CAVE2"},
  {"ID","BASE_CAVE3"},
  {"ID","BASE_CAVE4"},
  {"ID","BASE_CAVE5"},
  {"ID","BLD_FLYJAR"},
  {"ID","BLD_PLANET_HOLO"},
  {"ID","JETS_BUBBLE"},
  {"ID","S_POSTER12"},
  {"ID","S_POSTER14"},
  {"ID","S_POSTER16"},
  {"ID","SPEC_BB_TRACK01"},
  {"ID","SPEC_BB_TRACK02"},
  {"ID","SPEC_BB_TRACK03"},
  {"ID","DECAL_BIOME3"},
  {"ID","DECAL_CPLANET1"},
  {"ID","DECAL_STARTSHIP"},
  {"ID","EXPD_SPEC02"},
  {"ID","EXPD_TITLE02A"},
  {"ID","EXPD_TITLE02B"},
  {"ID","EXPD_DECAL02"},
  {"ID","EXPD_BANNER02"},
  {"ID","EXPD_FIREWORK01"},
  {"ID","EXPD_FIREWORK02"},
  {"ID","EXPD_FIREWORK03"},
  {"ID","S_SAIL0"},
  {"ID","S_SAIL1"},
  {"ID","S_SAIL2"},
  {"ID","S_SAIL3"},
  {"ID","BASE_NEXUS1"},
  {"ID","BASE_NEXUS2"},
  {"ID","BASE_NEXUS3"},
  {"ID","SPEC_FIREWORK04"},
  {"ID","SPEC_FIREWORK05"},
  {"ID","SPEC_FIREWORK06"},
  {"ID","SPEC_SPOOKYHEAD"},
  {"ID","SPEC_XOHELMET"},
  {"ID","BLD_EXPD_BASE01"},
  {"ID","EXPD_HELMET01"},
  {"ID","EXPD_BACKPACK01"},
  {"ID","EXPD_BANNER01"},
  {"ID","EXPD_DECAL01"},
  {"ID","EXPD_TITLE01"},
  {"ID","EXPD_SHIP01"},
  {"ID","TWITCH_SHIP01"},
  {"ID","TWITCH_SHIP02"},
  {"ID","TWITCH_SHIP03"},
  {"ID","TWITCH_SHIP04"},
  {"ID","TWITCH_SHIP05"},
  {"ID","TWITCH_GUN01"},
  {"ID","TWITCH_GUN02"},
  {"ID","TWITCH_GUN03"},
  {"ID","TWITCH_GUN04"},
  {"ID","TWITCH_GUN05"},
  {"ID","TWITCH_GUNUP01A"},
  {"ID","TWITCH_GUNUP01B"},
  {"ID","TWITCH_GUNUP01C"},
  {"ID","TWITCH_GUNUP01D"},
  {"ID","TWITCH_GUNUP01E"},
  {"ID","SPEC_TITLE25"},
  {"ID","SPEC_TITLE26"},
  {"ID","SPEC_TITLE27"},
  {"ID","BASE_LAVA1"},
  {"ID","BASE_LAVA2"},
  {"ID","BASE_LAVA3"},
  {"ID","MYSTERY_BEACON"},
  {"ID","BANNER_QIT"},
  {"ID","DECAL_AGT"},
  {"ID","DECAL_AMINO"},
  {"ID","DECAL_CAFE"},
  {"ID","DECAL_QIT"},
  {"ID","DECAL_UFT"},
  {"ID","SPEC_PETCUST1"},
  {"ID","SPEC_PETCUST2"},
  {"ID","SPEC_PETCUST3"},
  {"ID","BASE_HOUDINI1"},
  {"ID","BASE_HOUDINI2"},
  {"ID","BASE_HOUDINI3"},
  {"ID","SPEC_TITLE10"},
  {"ID","SPEC_TITLE21"},
  {"ID","SPEC_TITLE12"},
  {"ID","BASE_INFEST1"},
  {"ID","BASE_INFEST2"},
  {"ID","BASE_INFEST3"},
  {"ID","BANNER_AGT"},
  {"ID","BANNER_AMINO"},
  {"ID","BANNER_CAFE"},
  {"ID","BANNER_UFT"},
  {"ID","BASE_SWAMP1"},
  {"ID","BASE_SWAMP2"},
  {"ID","BASE_SWAMP3"},
  {"ID","SPEC_TITLE9"},
  {"ID","SPEC_TITLE8"},
  {"ID","SPEC_TITLE2"},
  {"ID","SHIP_DARK"},
  {"ID","SHIP_RAINBOW"},
  {"ID","BASE_DUSTPLANT1"},
  {"ID","BASE_DUSTPLANT2"},
  {"ID","BASE_DUSTPLANT3"},
  {"ID","SPEC_TITLE7"},
  {"ID","SPEC_TITLE6"},
  {"ID","SPEC_TITLE5"},
  {"ID","SPEC_TITLE4"},
  {"ID","BOBBLE_ART"},
  {"ID","BOBBLE_APOLLO"},
  {"ID","BOBBLE_NULL"},
  {"ID","SPEC_TITLE24"},
  {"ID","SPEC_TITLE1"},
  {"ID","SPEC_TITLE3"},
  {"ID","SHIP_RED"},
  {"ID","SHIP_GREEN"},
  {"ID","SHIP_GOLD"},
  {"ID","BOBBLE_ATLAS"},
  {"ID","BASE_COLDPLANT1"},
  {"ID","BASE_COLDPLANT2"},
  {"ID","BASE_COLDPLANT3"},
  {"ID","NEW_BACKPACK"},
  {"ID","BASE_TOXPLANT01"},
  {"ID","BASE_TOXPLANT02"},
  {"ID","BASE_TOXPLANT03"},
  {"ID","BANNER_CHU"},
  {"ID","BANNER_BLOB"},
  {"ID","BANNER_CHEF"},
  {"ID","BOBBLE_POLO"},
  {"ID","BOBBLE_NADA"},
  {"ID","BASE_RADPLANT01"},
  {"ID","BASE_RADPLANT02"},
  {"ID","BASE_RADPLANT03"},
  {"ID","BANNER_HUB"},
  {"ID","BANNER_POLO"},
  {"ID","BANNER_NADA"},
  {"ID","BASE_ROCK01"},
  {"ID","BASE_ROCK02"},
  {"ID","BASE_ROCK03"},
  {"ID","JETS_DARK"},
  {"ID","JETS_RAINBOW"},
  {"ID","JETS_RED"},
  {"ID","JETS_GREEN"},
  {"ID","JETS_BLUE"},
  {"ID","SPEC_EXOTIC1"},
  {"ID","SPEC_EXOTIC4"},
  {"ID","SPEC_EXOTIC9"},
  {"ID","SPEC_EXOTIC11"},
  {"ID","SPEC_EXOTIC5"},
  {"ID","SPEC_EXOTIC8"},
  {"ID","SPEC_EXOTIC12"},
  {"ID","SPEC_EXOTIC7"},
  {"ID","SPEC_EXOTIC6"},
  {"ID","SPEC_EXOTIC10"},
  {"ID","SPEC_EXOTIC3"},
  {"ID","SPEC_EXOTIC2"},
  {"ID","SPEC_VYK_ARMOUR"},
  {"ID","SPEC_VYK_TORSO"},
  {"ID","SPEC_VYK_LEGS"},
  {"ID","SPEC_VYK_GLOVES"},
  {"ID","SPEC_VYK_BOOTS"},
  {"ID","SPEC_HELMET05"},
  {"ID","BASE_TERRARIUM"},
  {"ID","BASE_HOTPLANT01"},
  {"ID","BASE_HOTPLANT02"},
  {"ID","BASE_HOTPLANT03"},
  {"ID","SPEC_HELMET03"},
  {"ID","BASE_TOYCORE"},
  {"ID","SPEC_EMOTE08"},
  {"ID","SPEC_EMOTE09"},
  {"ID","SPEC_EMOTE10"},
  {"ID","SPEC_FIREWORK03"},
  {"ID","SPEC_FIREWORK01"},
  {"ID","SPEC_FIREWORK02"},
  {"ID","BASE_WPLANT1"},
  {"ID","BASE_WPLANT2"},
  {"ID","BASE_WPLANT3"},
  {"ID","BASE_BARNACLE"},
  {"ID","SPEC_EMOTE05"},
  {"ID","BASE_AQUARIUM"},
  {"ID","STATUE_ATLAS_B"},
  {"ID","STATUE_ATLAS_S"},
  {"ID","STATUE_ATLAS_G"},
  {"ID","BASE_MEDPLANT01"},
  {"ID","BASE_MEDPLANT02"},
  {"ID","BASE_MEDPLANT03"},
  {"ID","SPEC_EMOTE04"},
  {"ID","STATUE_SHIP_B"},
  {"ID","STATUE_SHIP_S"},
  {"ID","STATUE_SHIP_G"},
  {"ID","BASE_TOYCUBE"},
  {"ID","BASE_TOYSPHERE"},
  {"ID","SPEC_DECAL07"},
  {"ID","SPEC_DECAL08"},
  {"ID","BASE_TREE01"},
  {"ID","BASE_TREE02"},
  {"ID","BASE_TREE03"},
  {"ID","BASE_ROBOTOY"},
  {"ID","SPEC_DECAL06"},
  {"ID","SPEC_EMOTE02"},
  {"ID","STATUE_DIP_B"},
  {"ID","STATUE_DIP_S"},
  {"ID","STATUE_DIP_G"},
  {"ID","SPEC_EMOTE03"},
  {"ID","SPEC_HELMET02"},
  {"ID","SPEC_DECAL04"},
  {"ID","SPEC_DECAL05"},
  {"ID","STATUE_WALK_B"},
  {"ID","STATUE_WALK_S"},
  {"ID","STATUE_WALK_G"},
  {"ID","STATUE_GEK_B"},
  {"ID","STATUE_GEK_S"},
  {"ID","STATUE_GEK_G"},
  {"ID","SPEC_EMOTE01"},
  {"ID","SPEC_HELMET01"},
  {"ID","SPEC_DECAL01"},
  {"ID","SPEC_DECAL02"},
  {"ID","SPEC_DECAL03"},
  {"ID","STATUE_BLOB_B"},
  {"ID","STATUE_BLOB_S"},
  {"ID","STATUE_BLOB_G"},
  {"ID","STATUE_ASTRO_B"},
  {"ID","STATUE_ASTRO_S"},
  {"ID","STATUE_ASTRO_G"}
}

NMS_MOD_DEFINITION_CONTAINER = 
{
["MOD_FILENAME"] 			= ModName,
["MOD_AUTHOR"]				= Author,
["NMS_VERSION"]				= GameVersion,
["MOD_DESCRIPTION"] 		= Description,
["MODIFICATIONS"] 			= 
  {
    {
      MBIN_CHANGE_TABLE 	= 
      { 
        {
          MBIN_FILE_SOURCE 	= "METADATA/REALITY/TABLES/NMS_REALITY_GCPRODUCTTABLE.MBIN",
          MXML_CHANGE_TABLE 	= 
          {
          }
        },
        {
          MBIN_FILE_SOURCE 	= "METADATA/REALITY/TABLES/NMS_MODULARCUSTOMISATIONPRODUCTS.MBIN",
          MXML_CHANGE_TABLE 	= 
          {
            {
              SPECIAL_KEY_WORDS =
              {
                {"ID","DROPS_COCKS13"},
                {"ID","DROPS_ENGIS13"},
                {"ID","DROPS_WINGS13"}
              },
              VALUE_CHANGE_TABLE  =
              {
                {"BaseValue",0},
              }
            },
          }
        },
        {
          MBIN_FILE_SOURCE 	= "METADATA/REALITY/TABLES/NMS_BASEPARTPRODUCTS.MBIN",
          MXML_CHANGE_TABLE 	= 
          {
            {
              SPECIAL_KEY_WORDS =
              {
                {"ID","B_TUR_B"},
                {"ID","B_DECO_Q_0"},
                {"ID","B_DECO_R"},
                {"ID","B_DECO_S"},
                {"ID","B_DECO_T"},
                {"ID","B_SHL_E"},
                {"ID","B_STR_AA_N"},
                {"ID","B_WNG_P"},
                {"ID","B_WNG_Q"},
                {"ID","B_WNG_R"},
              },
              VALUE_CHANGE_TABLE  =
              {
                {"BaseValue",0},
              }
            },
          }
        },
        {
          ["MBIN_FILE_SOURCE"] 	= "METADATA/REALITY/TABLES/REWARDTABLE.MBIN",
          ["MXML_CHANGE_TABLE"] 	= 
          {
            {
              SPECIAL_KEY_WORDS = 
              {
                {"Id","R_TGA_SHIP01"},
                {"Id","RS_S1_SHIP"},
                {"Id","RS_S9_SHIP"},
                {"Id","RS_S12_SHIP"},
                {"Id","RS_S13_SHIP"},
                {"Id","RS_S16_SHIP"},
                {"Id","RS_S17_SHIP"}
              },
              VALUE_CHANGE_TABLE  =
              {
                {"CostAmount",0},
              }
            },
          }
        },
      }
    }
  }
}

local ChangeProductBaseValue = NMS_MOD_DEFINITION_CONTAINER.MODIFICATIONS[1].MBIN_CHANGE_TABLE[1].MXML_CHANGE_TABLE

for i = 1, #ProductIDList do
	local ID = ProductIDList[i][1]
	local ProductName = ProductIDList[i][2]


			ChangeProductBaseValue[#ChangeProductBaseValue+1] =
			{
				SPECIAL_KEY_WORDS = {ID,ProductName},
				VALUE_CHANGE_TABLE  =
        {
          {"BaseValue",0},
        }
			}
end