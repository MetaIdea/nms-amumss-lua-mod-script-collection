NMS_MOD_DEFINITION_CONTAINER =
{
["MOD_FILENAME"]    = "HorribleTerror_Predators_1-Facil",
["MOD_AUTHOR"]      = "AldrichDDD",
["LUA_AUTHOR"]      = "Babscoole",
["NMS_VERSION"]     = "6.45",
["MOD_DESCRIPTION"] = "Predators hunt you in packs, spot you from further away, and never flee",
["MODIFICATIONS"]   =
  {
    {
      ["MBIN_CHANGE_TABLE"] =
      {
        {
          ["MBIN_FILE_SOURCE"] = "GCCREATUREGLOBALS.MBIN",
          ["MXML_CHANGE_TABLE"] =
          {
            {
              ["VALUE_CHANGE_TABLE"] =
              {
                {"MaxEcosystemCreaturesNormal",  "45"},
                {"PredatorPerceptionDistance",   "45.000000"},
                {"PercentagePlayerPredators",    "0.600000"},
                {"PredatorRunAwayHealthPercent", "30.000000"},
              },
            },
          }
        },
        {
          ["MBIN_FILE_SOURCE"] = "METADATA\SIMULATION\ECOSYSTEM\CREATUREGENERATIONDATA.MBIN",
          ["MXML_CHANGE_TABLE"] =
          {
            {
              ["SPECIAL_KEY_WORDS"] = {"Generic", "GcCreatureGenerationWeightedList", "Ground", "GcCreatureGenerationWeightedListDomainEntry"},
              ["SECTION_ACTIVE"] = 3,
              ["VALUE_CHANGE_TABLE"] =
              {
                {"Weight", "3.000000"},
              }
            },
            {
              ["PRECEDING_KEY_WORDS"] = {"GroundGroupsPerKm"},
              ["VALUE_CHANGE_TABLE"] =
              {
                {"Sparse",    "50.000000"},
                {"Normal",    "100.000000"},
                {"Dense",     "200.000000"},
                {"VeryDense", "400.000000"},
              }
            },
          }
        },
        {
          ["MBIN_FILE_SOURCE"] =
          {
            "METADATA\SIMULATION\ECOSYSTEM\GROUND\GROUNDTABLEPLAYERPREDATORLARGE.MBIN",
            "METADATA\SIMULATION\ECOSYSTEM\GROUND\GROUNDTABLEPLAYERPREDATORMED.MBIN",
          },
          ["MXML_CHANGE_TABLE"] = 
          {
            {
              ["VALUE_CHANGE_TABLE"] =
              {
                {"MinGroupSize", "1"},
                {"MaxGroupSize", "2"},
              }
            },
          }
        },
      }
    },
  }
}