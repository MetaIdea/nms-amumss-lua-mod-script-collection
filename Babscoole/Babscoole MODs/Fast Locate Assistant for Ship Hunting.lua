modName   = "Fast Locate Assistant for Ship Hunting"
batchName = ""
...
--]]
shipInfo = {
  {shipActive = huntExotics,    shipID = "ROYAL"},
  {shipActive = huntExplorers,  shipID = "SCIENTIFIC"},
  {shipActive = huntFighters,   shipID = "FIGHTER"},
  {shipActive = huntHaulers,    shipID = "DROPSHIP"},
  {shipActive = huntShuttles,   shipID = "SHUTTLE"},
  {shipActive = huntSolars,     shipID = "SAILSHIP"},
}
...
for i = 1, #shipInfo do
  if not shipInfo[i].shipActive then
    shipsToRemove[#shipsToRemove + 1] = {IDHandle, shipInfo[i].shipID}
  end
end
...
NMS_MOD_DEFINITION_CONTAINER  = {
  MOD_FILENAME    = table.concat({modName}),
  MOD_BATCHNAME   = batchName ~= "" and (batchName) or nil,
