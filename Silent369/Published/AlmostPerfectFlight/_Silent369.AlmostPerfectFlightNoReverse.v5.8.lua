local modfilename = "AlmostPerfectFlightNoReverse-RF"
local lua_author  = "Silent"
local lua_version = "5.8"
...
--Landing Curves
local landingCurveData = {
    {pkw = {"LandingCurve"},      vct = {{"Curve", "SmootherStep"}}, original = "SlowOut"},
    {pkw = {"LandingCurveHeavy"}, vct = {{"Curve", "SmootherStep"}}, original = "ReallySlowOut"},
    {pkw = {"LandingCurveWater"}, vct = {{"Curve", "SmootherStep"}}, original = "Linear"},
}

--Engine VCT data template (values multiplied by controlMult at insert time)
local engineVCTFields = {
...
local function insertMathVCT(target, vctData, mathOp, replaceType)
    local entry = {
        MATH_OP = mathOp,
        VCT = vctData,
    }
    if replaceType then
        entry.REPLACE_TYPE = replaceType
    end
    table.insert(target, entry)
end

local function insertVCT(target, vctData)
    table.insert(target, {VCT = vctData})
end

local function insertPKW(target, pkws, vctData)
    table.insert(target, {PKW = pkws, VCT = vctData})
end

local function insertSKWData(target, skw, vctData, mathOp, replaceType)
    local entry = {
        SKW = skw,
        VCT = vctData,
    }
    if mathOp then
        entry.MATH_OP = mathOp
    end
    if replaceType then
        entry.REPLACE_TYPE = replaceType
    end
...
if m_cheatySettings then
    insertMathVCT(TableData, cheatyData, "*")
end
...
if m_launchFuelCost then
    insertMathVCT(TableData, fuelSavingData, "*")
end
...
if m_warpSpeedMulti then
    insertMathVCT(TableData, miniWarpSpeedData, "*", "ALL")
end
...
local function insertEngineData(controlType, engineType, controlMult)
    local skw = {controlType, "GcPlayerSpaceshipControlData", engineType, "GcPlayerSpaceshipEngineData",}
    local vctData = {}
    for _, field in ipairs(engineVCTFields) do
        local name, baseVal = field[1], field[2]
        if name == "TurnStrength" then
            table.insert(vctData, {name, baseVal})
        else
            table.insert(vctData, {name, baseVal * controlMult})
        end
    end
    insertSKWData(TableData, skw, vctData, "*", "ALL")
end
...
local function insertHoverData(controlType)
    local skw = {controlType, "GcPlayerSpaceshipControlData",}
    insertSKWData(TableData, skw, hoverVCTData, nil, "ALL")
end
...
--------------------------------------------------------------------------------------------
-- General Data (Second MBIN_CT Block)
--------------------------------------------------------------------------------------------
...
insertMathVCT(GeneralData, noBoostData, "*", "ONCE")
insertMathVCT(GeneralData, miniWarpExitData, "*", "ONCE")
insertMathVCT(GeneralData, miniWarpStopData, "*", "ONCE")

--------------------------------------------------------------------------------------------
-- HUD Arrow Settings
--------------------------------------------------------------------------------------------

insertVCT(GeneralData, hudArrowData)

--------------------------------------------------------------------------------------------
-- General Settings
--------------------------------------------------------------------------------------------

insertVCT(GeneralData, generalData)

--------------------------------------------------------------------------------------------
-- Landing Curves
--------------------------------------------------------------------------------------------

for _, curve in ipairs(landingCurveData) do
    insertPKW(GeneralData, curve.pkw, curve.vct)
end
