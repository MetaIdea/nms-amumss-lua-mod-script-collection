...
--the distance between particles to appear less dense. It also scales up the gas/smoke in other biomes that
...
--GASSMOK is affecting the particles for the other particle
--        types: Gas, Smoke, Snow, Sparks, Embers, Flames...
--
...
local DIVIDER = 0     --Modifies the number of firefly particles.

local SPEED_V = 0.01  --Modifies the visible speed of particles.
local MULTPLY = 1.4   --Modifies the particles radius / radiusY.
local FADES_M = 0.5   --Modifies fade speed of rendered particles.
local SCALE_M = 1.5   --Modifies the x,y,z particles scale ranges.
local ROTAT_R = 0.5   --Modifies the rotational range of particles.

local SCALE_S = 1.2   --Modifies smoke/gas scale in certain biomes.
local GASSMOK = 0     --Modifies Sparks,Embers,Flames particle counts.
    ...
    {
        {
            ["MBIN_CHANGE_TABLE"] =
            {
                ...
                },
                {
                    ...
                    {
                        {
                            ["SPECIAL_KEY_WORDS"]   = {"Material", "MODELS/EFFECTS/COMMON/MATERIALS/SNOW.MATERIAL.MBIN"},
                            ["MATH_OPERATION"]      = "*",
                            ["INTEGER_TO_FLOAT"]    = "FORCE",
                            ["REPLACE_TYPE"]        = "ALL",
                            ["VALUE_MATCH"]         = "",
