...
local spawn_data = {
	props = {
		...
		c	= 'Coverage',				-- [*]
		l	= 'LodDistances',			-- [*]
		d	= 'FlatDensity',			-- [*] same applies to SlopeDensity
		f	= 'FadeOutStartDistance',	-- [*] same applies to FadeOutEndDistance
		r	= 'MaxRegionRadius',		-- [+]
	}
}
...
function spawn_data:averages()
	for k, d in pairs(self.mods) do
		self.res[k] = d.i > 0 and (d.v / d.i) or 1
	end
end
